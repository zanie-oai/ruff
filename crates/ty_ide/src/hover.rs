...
use ty_python_semantic::types::ide_support::{
    call_signature_details, resolved_call_signature, typed_dict_key_hover,
};
...
fn call_argument_hover_contents<'db>(
    ...
    let details = resolved_call_signature(model, call)?;
    ...
    let kind = parameter.kind.hover_label();
    let signature = has_multiple_call_signatures(model, call)
...
fn has_multiple_call_signatures(model: &SemanticModel, call: &ast::ExprCall) -> bool {
    call_signature_details(model, call).len() > 1
}
...
mod tests {
    ...
    #[test]
    fn hover_positional_argument_value() {
        let test = hover_test(
            r#"
    ...
    #[test]
    fn hover_double_starred_argument_unpack_multiple_parameters() {
        let test = hover_test(
            r#"
            from typing import TypedDict
            ...
            options: Options = {"first": 1, "second": "two"}
            def test(first: int, second: str):
                """my cool test
